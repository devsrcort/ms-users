...
actions.getUserInfo = async function(req, res, next) {
    ...
    log.info(`users : ${users[0]['email']} : ${users[0]['wallet_address']}`);
...
actions.sendTokenByClient = async function(req, res, next) {
    ...
    log.info(withdrawInfoService.isTestAccount(fromAddr));
    log.info(fromAddr);

    // if (!withdrawInfoService.isEnabled() || !withdrawInfoService.isTestAccount(fromAddr)) {
    if (!withdrawInfoService.isTestAccount(fromAddr)) {
        const response = {
            status: "NotEnabled"
        };
    ...
    tokenSender.sendToken(API_BASE, fromAddr, toAddr, amount, fee);
