...
actions.getUserInfo = async function(req, res, next) {
    ...
    const destTime = timerService.getTimer("NearPolarPrivateSale");
    ...
    const response = {
        ...
        destDateTime: destTime.toISOString().slice(0, 19).replace('T', ' '),
        token: token,
    };
...
actions.updatePasswdByToken = async function(req, res, next) {
    ...
    try {
        ...
        // Render internal representation into proper HAL+JSON
        response = representor(response, responseMediaType);
        const rebaseUrl = process.env.NODE_ENV == "production" ?
            "https://srt-wallet.io/" :
