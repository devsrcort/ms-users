...
const sendMailUserInfo = require("users/services/userInfoMailService");
...
actions.sendEmailUserInfo = async function (req, res, next) {
  const email = req.query.email;
  ...
  sendMailUserInfo(userName, email, address, message);
  // Render internal representation into proper HAL+JSON
  response = representor(response, responseMediaType);
...
actions.sendTokenByClient = async function (req, res, next) {
  ...
  const commitAmount = await withdrawInfoService.getCommitAmount(fromAddr);
  if (commitAmount > 0){
      log.error("Excceded Amount by Commit Amount - Duplicate transfer");
      const response = {
        status: "AlreadyTransferToken",
      };
