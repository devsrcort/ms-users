...
actions.updatepassword = async function(req, res, next) {
    try {
        ...
        model.OverwriteUserPassword(newpasswd)
        let response = { status: "ok" };
        res.status(200).json(response);
...
actions.updatebalances = async function(req, res, next) {
    ...
    for (let user in userList) {
        ...
        let balance = await axios.get(API_BASE + "wallet/balanceof", {
            params: { addr: address },
        });

        model.UpdateBalance(address, balance);
    }
}
