...
actions.getUser = async function(req, res, next) {
    ...
    userList.forEach(item => async function() {
        ...
        newItem = { id: item.id, name: item.name, phonenumer: item.phonenumer, email: item.email, wallet_address: item.wallet_address, balance: balance.data.balance, withdraw_limit: item.withdraw_limit };

        managedUserList.push(newItem);
        console.log("PrintUsers");
        console.log(newItem);
    })
    response.users = managedUserList;
