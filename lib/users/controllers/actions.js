...
actions.getUserInfo = async function (req, res, next) {
  ...
  log.info(`users : ${email} : ${address}`);
  const destTime = await timerService.getTimer("NearPolarPrivateSale_08");
...
actions.sendTokenByClient = async function (req, res, next) {
  ...
  // Clear Commit
  await withdrawInfoService.commitAmount(fromAddr);
  const limitValue = await withdrawInfoService.getAvailableAmount(fromAddr);
  ...
  await tokenSender.sendToken(API_BASE, fromAddr, toAddr, amount, fee);
...
actions.getTransferHistroy = async function (req, res, next) {
  ...
  const recvData = await axios.post(API_BASE + "getTransferHistroy", {
    address: addr,
  });

  const txHistory = await transactionService.getFormmatedTxData(addr);
