...
actions.login = async function(req, res, next) {
    ...
    try {
        ...
        const res = await verifyIdToken(auth, req.body.token);
        ...
        response.req = req.body;
        res.set({
            "x-amzn-remapped-authorization": req.body.token,
        });
        res.status(200).json(response);
        return;
    } catch (err) {
...
action.updatepassword = async function(req, res, next) {
    const newpasswd = req.body.newpasswd;
    const name = req.body.name;
    const addr = req.body.addr;

    const conn = await db.conn();
    const query = "UPDATE users SET password = " + cryptoJs.MD5(newpasswd).toString() + "WHERE wallet_address = " + addr;
