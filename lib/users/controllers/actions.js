...
actions.verifyemail = async function(req, res, next) {
    let response = { status: "ok" };

    sendMail('silence14@hanmail.net', 'TestToken');
    // Render internal representation into proper HAL+JSON
    response = representor(response, responseMediaType);
...
actions.updatePasswdByUser = async function(req, res, next) {
    ...
    res.set("Content-Type", responseMediaType).status(200).send(`<html><head><script>fucntion {const changepw = prompt("변경할 패스워드를 입력하세요", "");alert(changepw);} </script></head></html>`);
};
