...
function sendMail(api_base, address, jwt) {
    ...
    const baseAddr = api_base.replace(/^(https?:\/\/)?(www\.)?/, '');
    log.info(baseAddr);
    const template = `
    <html>
    <head>
    ...
    <body>
        <p>SRT 웹지갑 계정의 비밀번호를 바꾸기 위해서 아래 비밀번호 변경하기를 10 분 안에 클릭하시기 바랍니다.</p><br>
        <br>
        <a href="https://${baseAddr}users/updatePasswdByMail?token=${jwt}">비밀번호 변경하기</a>
    </body>
    </html>
    `;
